<x-layout>
    <div class="mx-auto max-w-3xl rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <h1 class="mt-3 text-3xl font-semibold text-slate-900">Create ingredient specification</h1>
        ...
        <form action="{{ route('ingredient-specifications.store') }}" method="POST" class="mt-8 grid gap-5 md:grid-cols-2">
            @csrf
            ...
            <div>
                ...
                <select name="nutrition_profile" id="nutrition_profile" required class="mt-2 block w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none transition focus:border-teal-400 focus:bg-white">
                    ...
                    @foreach ($nutritionProfiles as $nutritionProfile)
                        <option value="{{ $nutritionProfile->id }}" {{ old('nutrition_profile') == $nutritionProfile->id ? 'selected' : '' }}>{{ $nutritionProfile->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="number" step="0.01" id="cost_per_item" name="cost_per_item" value="{{ old('cost_per_item') }}" class="mt-2 block w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none transition focus:border-teal-400 focus:bg-white">
            </div>

            <div>
                <label for="item_size" class="block text-sm font-medium text-slate-700">Item Size</label>
                <input type="number" step="0.01" id="item_size" name="item_size" value="{{ old('item_size') }}" class="mt-2 block w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none transition focus:border-teal-400 focus:bg-white">
            </div>
            ...
            <div>
                ...
                <select name="unit_id" id="unit_id" class="mt-2 block w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none transition focus:border-teal-400 focus:bg-white">
                    ...
                    @foreach ($units as $unit)
                        <option value="{{ $unit->id }}" {{ old('unit_id') == $unit->id ? 'selected' : '' }}>{{ $unit->unit_name }} ({{ $unit->unit_abbreviation }})</option>
                    @endforeach
                </select>
            </div>

            <div class="md:col-span-2">
                <button type="submit" class="flex w-full justify-center rounded-2xl bg-teal-500 px-4 py-3 text-sm font-medium text-white shadow-sm transition hover:bg-teal-600">Create Ingredient Specification</button>
            </div>
        </form>
    </div>
</x-layout>
