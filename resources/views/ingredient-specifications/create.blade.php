...
<html>
<body>
    <h1>Create Ingredient Specification</h1>
    ...
    <form action="{{ route('ingredient-specifications.store') }}" method="POST">
        @csrf
        ...
        <div>
            ...
            <select name="nutrition_profile" id="nutrition_profile" required>
                ...
                @foreach ($nutritionProfiles as $nutritionProfile)
                    <option value="{{ $nutritionProfile->id }}" {{ old('nutrition_profile') == $nutritionProfile->id ? 'selected' : '' }}>
        ...
        <div>
            <button type="submit">Create Ingredient Specification</button>
        </div>
    </form>
</body>
</html>