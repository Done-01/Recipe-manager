<x-layout>
    <div class="mx-auto max-w-2xl rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <h1 class="mt-3 text-3xl font-semibold text-slate-900">Create nutrition profile</h1>
        ...
        <form action="{{ route('nutrition-profiles.store') }}" method="POST" class="mt-8 space-y-5">
            @csrf
            ...
            <button type="submit" class="flex w-full justify-center rounded-2xl bg-teal-500 px-4 py-3 text-sm font-medium text-white shadow-sm transition hover:bg-teal-600">Create Nutrition Profile</button>
        </form>
    </div>
</x-layout>
